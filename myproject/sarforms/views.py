...
# Dit is de eerste pagina waar je een incident kan aanmaken. Dit is de pagina waar je de incidenten kan aanmaken en daaronder de ingevulde logs.
...
def radio_log(request):
    ...
    if request.method == "POST":
        ...
        form = Form133Form(initial={
            'incident_nr': volgend_incident_nr})
...
# dit is de volgende pagina waar vervolgens het log formulier staat en daaronder de ingevulde logs.
...
def radio_log_combined(request):
    ...
    form = Form133NextForm(initial={
        'incident_nr': max_incident_nr,
        'datum': now.date(),
        'tijd': now.time().strftime('%H:%M:%S'),
